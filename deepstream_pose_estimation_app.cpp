...
{
  int K = topology.size();
  //cout << "topology size : " << K << endl;
  int count = objects.size();
  //cout << "objects size : " << count << endl;
  ...
  {
    int C = object.size();
    //cout << "object size : " << C << endl;
    for (int j = 0; j < C; j++)
    {
      int k = object[j];
      //cout << "object[" << j << "] : " << k << endl;
      if (k >= 0)
      {
        auto &peak = normalized_peaks[j][k];
        int x = peak[1] * MUXER_OUTPUT_WIDTH;
        int y = peak[0] * MUXER_OUTPUT_HEIGHT;
        //cout << "normalized peaks[" << j << "][" << k << "], " << "(" << x << "," << y << ")" << endl;

        json_output["frame"][frame_number]["object"][object_number].push_back({x, y});
      ...
      {
        json_output["frame"][frame_number]["object"][object_number].push_back({nullptr, nullptr});
      }
    }
...
/* Add a transform element for Jetson*/
  ...
  json_output["resolution"]["width"] = MUXER_OUTPUT_WIDTH;
  json_output["resolution"]["height"] = MUXER_OUTPUT_HEIGHT;
  json_output["keypoints"] = { "nose", "left_eye", "right_eye", "left_ear", "right_ear", "left_shoulder", "right_shoulder", "left_elbow", "right_elbow", "left_wrist", "right_wrist", "left_hip", "right_hip", "left_knee", "right_knee", "left_ankle", "right_ankle", "neck" };
  json_output["skeleton"] = { {15, 13}, {13, 11}, {16, 14}, {14, 12}, {11, 12}, {5, 7}, {6, 8}, {7, 9}, {8, 10}, {1, 2}, {0, 1}, {0, 2}, {1, 3}, {2, 4}, {3, 5}, {4, 6}, {17, 0}, {17, 5}, {17, 6}, {17, 11}, {17, 12} };

  std::ofstream file("/root/Music/result.json");
  file << json_output;
